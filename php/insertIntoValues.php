...
try {
	$sql="INSERT INTO Room (Building, Floor, RoomNo, Capacity) VALUES (:buildingNo, :floorNo, :roomNo, :capacity);";   // all the variable names must start with a colon (:)
	$stmnt = $conn->prepare($sql);    // read about prepared statement here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
	$stmnt->bindParam(':buildingNo', $_POST['buildingNo']);   // stdId in $_POST['stdId'] in the exact name of the input element in HTML. if any typo, your code does not work   
	$stmnt->bindParam(':floorNo', $_POST['floorNo']);   // note the single quotes, If you forget to put single quotes, your code does not work.
	$stmnt->bindParam(':roomNo', $_POST['roomNo']);
	$stmnt->bindParam(':capacity', $_POST['capacity']);
